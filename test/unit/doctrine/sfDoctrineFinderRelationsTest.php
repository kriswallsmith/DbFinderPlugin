...
$t = new lime_test(80, new lime_output_color());
...
$article3->save();
$nbArticles = sfDoctrineFinder::from('DArticle')->join('DCategory')->where('DCategory.Name', 'cat1')->count();
$t->is($nbArticles, 2, 'join() allows to join to another table (many-to-one)');
$nbArticles = sfDoctrineFinder::from('DArticle')->where('DCategory.Name', 'cat1')->count();
$t->is($nbArticles, 2, 'join() can be omitted if column names are explicit (many-to-one)');
$nbArticles = sfDoctrineFinder::from('DArticle')->join('DCategory')->where('DCategory.Name', 'cat2')->count();
...
$article = sfDoctrineFinder::from('DArticle')->join('DCategory')->where('DCategory.Name', 'cat2')->findOne();
...
$comment->save();
$nbArticles = sfDoctrineFinder::from('DArticle')->join('DComment')->where('DComment.Content', 'foo')->count();
$t->is($nbArticles, 1, 'join() allows to join to another table (one-to-many)');
$nbArticles = sfDoctrineFinder::from('DArticle')->where('DComment.Content', 'foo')->count();
$t->is($nbArticles, 1, 'join() can be omitted if column names are explicit (one-to-many)');
$article = sfDoctrineFinder::from('DArticle')->join('DComment')->where('DComment.Content', 'foo')->findOne();
$t->is($article->getTitle(), 'bbbbb', 'join() allows to join to another table (one-to-many)');

$nbArticles = sfDoctrineFinder::from('DArticle')->join('Category')->where('Category.Name', 'cat1')->count();
$t->is($nbArticles, 2, 'join() accepts relation names instead of related class names');
$nbArticles = sfDoctrineFinder::from('DArticle')->join('DCategory a')->where('a.Name', 'cat1')->count();
$t->is($nbArticles, 2, 'join() accepts class aliases');
$nbArticles = sfDoctrineFinder::from('DArticle')->join('Category a')->where('a.Name', 'cat1')->count();
$t->is($nbArticles, 2, 'join() accepts relation aliases');

$finder = sfDoctrineFinder::from('DArticle')->join('DCategory');
$finder->count();
$t->ok(stripos($finder->getLatestQuery(), 'inner join') !== false, 'join($table) defaults to an inner join');
$finder = sfDoctrineFinder::from('DArticle')->join('DCategory', 'left join');
$finder->count();
$t->ok(stripos($finder->getLatestQuery(), 'left join') !== false, 'join($table, $type) accepts a join type as second parameter (like "left join")');
$finder = sfDoctrineFinder::from('DArticle')->join('DCategory', Criteria::LEFT_JOIN);
$finder->count();
$t->ok(stripos($finder->getLatestQuery(), 'left join') !== false, 'join($table, $type) accepts a join type as second parameter (like "Criteria::LEFT_JOIN")');
$finder = sfDoctrineFinder::from('DArticle')->join('DCategory', 'left');
$finder->count();
$t->ok(stripos($finder->getLatestQuery(), 'left join') !== false, 'join($table, $type) accepts a join type as second parameter (like "left")');

$finder = sfDoctrineFinder::from('DArticle')->join('DArticle.CategoryId', 'DCategory.Id', 'inner')->where('Category.Name', 'cat1');
$t->is($nbArticles, 2, 'join($start, $end, $type) allows to join based on the two members of the relationship');
sfDoctrineFinder::from('DArticle')->join('DCategory.Id', 'DArticle.CategoryId', 'inner')->where('Category.Name', 'cat1');
...
$comment->save();
$article = sfDoctrineFinder::from('DArticle')->join('DComment')->join('DAuthor')->where('DAuthor.Name', 'John')->findOne();
